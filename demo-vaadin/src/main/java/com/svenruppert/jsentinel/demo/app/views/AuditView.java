...
import com.svenruppert.jsentinel.audit.ApiKeyDenied;
import com.svenruppert.jsentinel.audit.ApiKeyUsed;
...
/**
 * Standalone Vaadin route showing the in-memory ring buffer of audit
 * events. Restricted at view level by
 * {@link RequiresPermission @RequiresPermission("audit:read")}, granted
 * to {@code ADMIN} and {@code Q_ADMIN} via
 * {@code MyAuthorizationService}.
 */
@Route(AuditView.NAV)
...
public class AuditView extends Composite<VerticalLayout> {
  ...
  private void refresh() {
    ...
    // Show newest first — query returns oldest first.
    List<AuditEvent> reversed = new java.util.ArrayList<>(events);
  ...
  private static String subjectOf(AuditEvent event) {
    return switch (event) {
      ...
      case com.svenruppert.jsentinel.audit.CredentialVerificationSucceeded e -> e.username();
      case com.svenruppert.jsentinel.audit.CredentialVerificationFailed e -> e.username();
      case com.svenruppert.jsentinel.audit.CredentialRehashed e -> e.username();
      case com.svenruppert.jsentinel.audit.CredentialStatusChanged e -> e.username();
    };
  }
