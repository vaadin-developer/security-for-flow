 ...
 * single {@link JCustosSubject} and binds the subject as the current
 ...
 * FakeAuthZ + SubjectStore + JCustosServiceResolver scaffolding by
 * hand.
 *
 * <pre>
 ...
 *
 * <pre>
 *   JCustosTestFixture.runAs(adminSubject, () -> {
 *     // act-and-assert
 *   });
 * </pre>
 *
 * @since 00.74.00
 */
public final class JCustosTestFixture implements AutoCloseable {

  private final JCustosSubject subject;

  private JCustosTestFixture(JCustosSubject subject) {
    this.subject = subject;
  }

  /** @return the bound subject */
  public JCustosSubject subject() {
    return subject;
  }
   ...
   *
   * <p>Requires that {@link JCustosServiceResolver} already has a
   * {@link eu.jsentinel.jcustos.authentication.AuthenticationService}
   * + {@link eu.jsentinel.jcustos.authorization.api.AuthorizationService}
   * wired for {@link JCustosSubject}. For a self-contained setup,
   ...
   */
  public static void runAs(JCustosSubject subject, Runnable block) {
    Objects.requireNonNull(subject, "subject");
    Objects.requireNonNull(block, "block");
    SubjectStores.subjectStore().setCurrentSubject(subject, JCustosSubject.class);
    try {
      block.run();
    } finally {
      SubjectStores.findSubjectStore()
  ...
  public static final class Builder {
    ...
    public JCustosTestFixture build() {
      ...
      JCustosSubject subject = new JCustosSubject(subjectId, name, roles, permissions);

      FakeAuthenticationService<String, JCustosSubject> authn =
          FakeAuthenticationService.forType(JCustosSubject.class);
      FakeAuthorizationService<JCustosSubject> authz = new FakeAuthorizationService<>();
      ...
      store.setCurrentSubject(subject, JCustosSubject.class);

      return new JCustosTestFixture(subject);
    }
  }
}
