...
/**
 * Internal {@link SecurityAuditService} that forwards every
 ...
 * only.
 *
 * <p>Used by {@link AbstractSecurityBootstrap} when the fluent
 * {@code .audit(...)} configuration mixes a
 * {@code StoreBackedSecurityAuditService} with one or more sink-only
 ...
 * @since 00.73.00
 */
final class TeeingSecurityAuditService implements SecurityAuditService {

  private final SecurityAuditService primary;
  private final List<SecurityAuditService> siblings;

  TeeingSecurityAuditService(SecurityAuditService primary, SecurityAuditService... siblings) {
  ...
  public void publish(AuditEvent event) {
    ...
    for (SecurityAuditService sibling : siblings) {
      try {
  ...
  SecurityAuditService primary() {
    return primary;
  }

  /** Test/diagnostics helper. */
  List<SecurityAuditService> siblings() {
    return siblings;
  }
}
