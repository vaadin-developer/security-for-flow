...
public final class SecurityDiagnostics {
  ...
  public static SecurityServiceReport inspect() {
    ...
    if (cl == null) {
      cl = SecurityDiagnostics.class.getClassLoader();
    }
    ...
    for (Class<?> spi : CORE_SPIS) {
      ...
      if (impls.isEmpty() && CRITICAL.contains(spi)) {
        builder.addMissing(new MissingRecommendedService(
            ...
            "Register an implementation via @SecurityAutoService(" + spi.getSimpleName()
    ...
    // 2. derived rule: SecurityVersionStore present but SubjectIdResolver missing
    boolean hasVersionStore = builder.discovered.stream()
        .anyMatch(d -> d.spi() == SecurityVersionStore.class);
    ...
    if (hasVersionStore && !hasSubjectIdResolver) {
      builder.addWarning(new ServiceWarning(
          ...
          "SecurityVersionStore is registered, but no SubjectIdResolver was found.",
          "Register a SubjectIdResolver via @SecurityAutoService("
