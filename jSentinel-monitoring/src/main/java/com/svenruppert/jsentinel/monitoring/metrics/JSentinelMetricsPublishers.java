 ...
 * {@link JSentinelMetricsPublisher} implementations.
 *
 * <p>Resolution rules: no registered implementation → the
 * {@link NoOpJSentinelMetricsPublisher#INSTANCE no-op publisher}
...
public final class JSentinelMetricsPublishers {

  private JSentinelMetricsPublishers() {
  }

  /**
   * Discovers the effective {@link JSentinelMetricsPublisher} via
   * {@link ServiceLoader}. Never throws.
   *
   * @return the discovered publisher, or
   *     {@link NoOpJSentinelMetricsPublisher#INSTANCE} when none is
   *     registered or discovery fails
   */
  public static JSentinelMetricsPublisher discover() {
    List<JSentinelMetricsPublisher> providers = new ArrayList<>();
    try {
      for (JSentinelMetricsPublisher provider : ServiceLoader.load(JSentinelMetricsPublisher.class)) {
      ...
      return NoOpJSentinelMetricsPublisher.INSTANCE;
    }
   ...
   */
  static JSentinelMetricsPublisher select(List<JSentinelMetricsPublisher> providers) {
