# Skill-Feedback: `jsentinel-vaadin-persistence`
...
> Refactor wurde auf `jsentinel-rest-persistence` und
> `jsentinel-standalone-persistence` durchgezogen. Smoke-Test
> (close → reopen → reload) ist für alle drei Adapter grün.
> Demo-Module: alle 9 (`demo-jsentinel-{vaadin,rest,standalone}{,-persistence,-hardening}`) kompilieren weiter.
...
(`NotSerializableException` mit Stacktrace) war im jSentinel-Core
...
weiterfliegt — sonst wird der Trace im jSentinel-Catch verschluckt.
...
**Anregung an jSentinel-Core** (nicht Teil des Skill-Fixes): der
...
jSentinel-Eclipse-Store unter demselben Datenverzeichnis. Das ist
...
- **Exception-Pattern**: `NotSerializableException` wird vom Skill
  selber nicht abgefangen und dann vom jSentinel-Catch verschluckt
  (siehe Bug 2).
...
Spiegel-Fix auch in `jsentinel-rest-persistence` und
`jsentinel-standalone-persistence` durchgezogen — dieselben sechs
Templates, dieselbe Persistence-Layer-Topologie. Die drei Hardening-
Demos (`demo-jsentinel-*-hardening`) wurden ebenfalls re-rendert, weil
