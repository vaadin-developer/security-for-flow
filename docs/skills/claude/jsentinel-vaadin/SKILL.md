---
name: jsentinel-vaadin
description: Battle-tested integration of jSentinel (V00.73+) into a Vaadin Flow application. Wires the full security stack — V00.73 fluent `VaadinSecurity.bootstrap()` + `@JSentinelAutoService`-registered `AuthenticationService` / `AuthorizationService` + pre-seeded in-memory admin + regular user + `LoginView` extending `com.svenruppert.jsentinel.authorization.LoginView` + `LoginListener` + AppLayout `MainLayout` shell hosting both **public** and **private** routes: a `PublicHomeView` at `/` reachable without login + a `DashboardView` at `/dashboard` (`@VisibleFor(USER)`) after login + the admin views as further children via `@Route(layout = MainLayout.class)`. Drawer uses a plain `RouterLink` for public entries and `SecuredUi.link().hideWhenDenied()` for private ones — anonymous visitors see only "Home", logged-in users see Dashboard + permission-granted admin areas. Navbar carries a dynamic Sign in / Sign out button rebuilt via `BeforeEnterObserver`. Admin views: audit (`SecurityAuditService.query` over a ring-buffer / store-backed sink, filterable by type / subject / from-to DateTimePicker) + sessions (`SessionManagementView` over an `InMemorySessionStore`) + role management (`@RequiresPermission("admin:roles")`). Includes `HomeButton` helper that conditionally renders a back-to-`/` button only when a view is standalone (`@Route(layout = UI.class)`). Use PROACTIVELY when the project shows Vaadin Flow signals (`vaadin-core`, `@Route`, `VerticalLayout`, `AppLayout`, `Composite`) AND the user mentions jSentinel, integrate jSentinel, jSentinel einbauen, secure my Vaadin app, Vaadin Security, security-for-flow, add login + roles, wire authentication, `VaadinSecurity.bootstrap`, `@SecureRoute`, `SecuredUi`, `JSentinelServiceResolver`, `JSentinelSubject`, audit view, session inventory, admin roles UI. Also applies when the user asks to "bootstrap jSentinel in a fresh Vaadin app", "seed admin + regular user", "add Audit + Session admin views", "give me a complete jSentinel integration", or wants the full V00.73 fluent surface (`audit / sessions / policies / roles / credentials` sub-builders) rather than the legacy hand-written `META-INF/services` recipe. Provides the 5-step recipe (POM patch → typed slot extraction → source generation → bootstrap-init listener registration → verify with `mvn compile` + dev-run), 17 ready-to-render `.java.tmpl` templates with placeholder substitution, the discipline of "Authn + Authz via `@JSentinelAutoService`, LoginListener + AccessEvaluator + VaadinServiceInitListener via `META-INF/services`", and the pitfall catalog (annotation processor path order, `optimizeBundle=false`, subject type leakage, audit sink failure swallowing). Does NOT cover token-based first-admin bootstrap (`BootstrapWiring` / `SetupView`), credential persistence (use `jSentinel-persistence-eclipsestore`), or REST/Standalone integration (separate skills).
---

# jSentinel ↦ Vaadin Flow — full integration

This skill drops the complete jSentinel security stack into a Vaadin
...
- an `/audit` view restricted by `@RequiresPermission("audit:read")`
  that queries `JSentinelServiceResolver.securityAuditService()` and
...
1. **Read the user's brief.** Most briefs name two things: "add
   jSentinel" + the app's package. Extract the slots in
...
│   │   ├── VisibleFor.java               ← @JSentinelAnnotation(RoleAccessEvaluator.class)
...
│   ├── services/
│   │   ├── My<...>AuthenticationService.java ← @JSentinelAutoService
│   │   ├── My<...>AuthorizationService.java  ← @JSentinelAutoService
│   │   └── SessionStoreProvider.java
│   └── bootstrap/
│       └── JSentinelBootstrapInitListener.java ← VaadinServiceInitListener
└── views/
...
├── com.svenruppert.jsentinel.authorization.api.AccessEvaluator
...
|---|---|---|
| `AuthenticationService`, `AuthorizationService` | `@JSentinelAutoService` | Generated `META-INF/services` entry — no manual file maintenance |
| `AccessEvaluator` | hand-written `META-INF/services` line | Loaded by `JSentinelAnnotationScanner`, but `@JSentinelAutoService` only ships for `AuthenticationService` / `AuthorizationService` in V00.73 |
| `LoginListener` | hand-written `META-INF/services` line | Same |
| `VaadinServiceInitListener` (the bootstrap init) | hand-written `META-INF/services` line | Vaadin's own SPI mechanism, outside the `@JSentinelAutoService` scope |

Don't try to convert the second group to `@JSentinelAutoService` — the
...
carries no `@JSentinelAnnotation`-meta-annotated restriction, the
...
| `JSentinelBootstrapInitListener.java.tmpl` | `src/main/java/{{BASE_PATH}}/security/bootstrap/JSentinelBootstrapInitListener.java` |
...
| `services-LoginListener.tmpl` | `src/main/resources/META-INF/services/com.svenruppert.jsentinel.authorization.LoginListener` |
| `services-AccessEvaluator.tmpl` | `src/main/resources/META-INF/services/com.svenruppert.jsentinel.authorization.api.AccessEvaluator` |
...
`jSentinel-parent` (or pull `<version>` from a property — the
...
Define `<jsentinel.version>` in `<properties>` if not inherited. Use
...
startup banner via `JSentinelRuntime.log()` to stdout, wires an
...
`strictDefaults` raises `JSentinelBootstrapException` if any critical
...
`JSentinelBootstrapInitListener` and recompile.
...
2. Verify `META-INF/services` has three new entries:
   ...
   ```
3. Verify `@JSentinelAutoService` produced the `META-INF/services`
...
The bootstrap banner (a multi-line `JSentinelRuntime.log()` printout)
...
later adds the REST adapter (`jSentinel-rest`), DO NOT reuse the same
type — REST has its own `JSentinelSubject` view of the user. The
`AuthenticationService` is the boundary; everything past
`loadSubject` is adapter-local.

### Audit sink swallowing failures

The `JSentinelAuditService` calls in the templates wrap every
...
through `JSentinelServiceResolver.passwordHashingService()` before
...
### `<jsentinel.version>` property

If the consuming project doesn't already define `<jsentinel.version>`
(or `${project.version}` for in-reactor builds), the rendered POM
fails. The skill writes `${jsentinel.version}` by default — if the
...
- **Persistence** (`jSentinel-persistence-eclipsestore`,
  `StoreBackedJSentinelAuditService`, `EclipseStoreSessionStore`).
