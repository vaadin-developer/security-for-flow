...
- **`@Secured` + `<Type>Secured`** — production hot path, no interface,
  ...
  through `SecurityDiagnostics.inspect()`.
...
- **Sub-builders** — `.audit(...)` composes the existing
  `LoggingAuditSink` / `RingBufferAuditSink` / `StoreBackedSecurityAuditService`;
...
- Don't put `PasswordHashingService` through
  `SecurityServiceResolver.setPasswordHashingService(...)` — that
...
- Don't call `.roles(r -> r.mapping(...))` — `RolePermissionMapping` is
  ...
  Register it directly through `SecurityServiceResolver` if needed.
