...
Timestamp: 2026-06-15_00-49-31 Europe/Berlin
State after V00.74.10 release (deployed to Maven Central) + V00.74.20 Phase 0 (reactor bumped to `00.74.20-SNAPSHOT`, bcprov 1.78.1 → 1.84 CVE fix).

Released coordinates on Maven Central: `com.svenruppert.jsentinel:*:00.74.10`.
Development line: `00.74.20-SNAPSHOT` on `develop`.
...
| Credential Pipeline | Argon2id / bcrypt / scrypt providers in opt-in module `jSentinel-crypto-bc` (BouncyCastle 1.84 as of V00.74.20 P0; no global JCA provider registration) | Shipped / V00.71; bcprov bumped V00.74.20 |
...
| Compliance | Serialization policy — explicit no-Java-serialization stance (CWE-502 sidestep); 4 legitimate `Serializable` shapes documented (Vaadin lifecycle + `Throwable` heritage); Eclipse-Store / Canonical JSON / JOSE codec choice | Shipped / V00.74.10 (docs/security/credentials/standards/serialization-policy.md) |
...
| Demos (V00.74) | 10 `demo-jsentinel-*` skill demos (rest, rest-hardening, rest-persistence, standalone, standalone-hardening, standalone-persistence, vaadin, vaadin-hardening, vaadin-persistence, vaadin-mfa) shipped with V00.74.00; parent-pom bump to V00.74.20-SNAPSHOT done in Phase 0 of V00.74.20 (was 00.73.00 in V00.74.10) | Shipped / V00.74; aligned V00.74.20 |
...
| Documentation | `Konzept-V00.74.20.md` + `Implementierungsplan-V00.74.20.md` (Storage-Pair Option B + V00.74.10 cleanup fold-in) | Shipped / V00.74.20 P0 |
| Documentation | `docs/estimates/google-login-effort-estimate.md` — three pathway analysis (Spike A 3–5 d, Lite B 12–18 d, Roadmap C ≈ 9–14 months over V00.76–V00.79) | Shipped / 2026-06-15 |
| Security Hygiene | bcprov-jdk18on 1.78.1 → 1.84 — closes Dependabot #15 (HIGH GHSA-p93r-85wp-75v3 / CVE-2026-5598 FrodoEngine timing channel) and #14 (MED GHSA-c3fc-8qff-9hwx / CVE-2026-0636 LDAPStoreHelper injection); neither code path is exercised in jSentinel | Shipped / V00.74.20 P0 |
| V00.74.20 (in flight) | Storage-Pair architecture (Option B) — `StorageLayout`, `JSentinelStoragePair`, `JSentinelStorageFactory` Public-API skeleton | Phase 0 done; Phase 1+ planned |
| V00.74.20 (cleanup, folded) | Coordinated 10× demo-jsentinel-* parent-pom bump 00.73.00 → 00.74.20-SNAPSHOT | Phase 0 done (36 poms total) |
| V00.74.20 (cleanup, folded) | HealthView demo on top of `JSentinelRuntime.healthCheck()` + `JSentinelBootstrapInitListener.currentRuntime()` accessor | Phase 6 planned |
| V00.74.20 (cleanup, folded) | Mutation-lift quick wins: `jSentinel-dx-standalone` 63 % → ≥ 65 %, `jSentinel-autoservice-processor` 52 % → ≥ 65 % | Phase 6 planned |
| V00.74.20 (non-scope) | No lift on `jSentinel-dx-vaadin` (53 % → 75 %) or `jSentinel-vaadin-starter` (38 % → 75 %) — backlog past V00.74.20; gaps are structural, not incremental | Documented in Konzept-V00.74.20 §7a |
...
| V00.80 | Monitoring / metrics / health | Partially in V00.71 (`CredentialSecurityMetrics` SPI); `JSentinelRuntime.healthCheck()` shipped in V00.74.10; export adapters Planned |
| V00.80 | Fail-closed strict mode | Delivered in V00.72 (`JSentinelBootstrapMode.STRICT` + `JSentinelBootstrapException`) |
| V00.80 | Supply-chain / release hardening | Partially in V00.71 (CycloneDX SBOM per module, FIPS profile docs, SBOM verification expectations); serialization-policy.md added V00.74.10 |
