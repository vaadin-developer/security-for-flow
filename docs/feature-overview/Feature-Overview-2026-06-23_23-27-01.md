...
Timestamp: 2026-06-23_23-27-01 Europe/Berlin
State after V00.74.20 release (published to Maven Central on 2026-06-23, Sonatype deployment ID `594cdd77-bb48-439b-bb26-ddcda8cd4c65`, GitHub release at https://github.com/vaadin-developer/security-for-flow/releases/tag/v00.74.20).

Released coordinates on Maven Central: `com.svenruppert.jsentinel:*:00.74.20`.
Development line: `00.74.20` on `develop` (`origin/develop` synchronised at `d2a54408`).
...
| Rebrand (V00.73) | jSentinel-themed Javadoc (`docs/javadoc/jsentinel.css` since V00.74.20, was `build/javadoc/jsentinel.css` in V00.73-V00.74.10; Light/Dark/System toggle drives the lime palette) | Shipped / V00.73; path moved V00.74.20 |
...
| Runtime Tooling (V00.74.10) | `JSentinelRuntime.summary()` — single-line banner `OK \| N services \| E errors \| W warnings \| I INFO` for log-aggregator filtering | Shipped / V00.74.10 |
| Runtime Tooling (V00.74.10) | `JSentinelRuntime.toMap()` — insertion-order `Map<String,Object>` via `Collections.unmodifiableMap(LinkedHashMap)` for structured-log enrichment | Shipped / V00.74.10 |
| Runtime Tooling (V00.74.10) | `JSentinelRuntime.toJson()` — RFC 8259 JSON via in-tree `JsonEncoder`, no JSON-library dependency on compile/runtime | Shipped / V00.74.10 |
| Runtime Tooling (V00.74.10) | `JSentinelRuntime.healthCheck()` returns `HealthStatus(HEALTHY/DEGRADED/FAILED, Instant, List<HealthFinding>)` for liveness/readiness probes | Shipped / V00.74.10 |
| Runtime Tooling (V00.74.10) | Maven Enforcer ban on Jackson / Gson / org.json on `jSentinel-dx` compile/runtime; test scope permitted (Jackson cross-validates `toJson()` output) | Shipped / V00.74.10 |
| Runtime Tooling (V00.74.20) | `JSentinelBootstrapInitListener.currentRuntime()` accessor — `private static volatile JSentinelRuntime runtime` field populated at the end of `serviceInit(...)`; admin views can re-read on every render so a future runtime swap is visible without a page reload | Shipped / V00.74.20 (demo-jsentinel-vaadin-hardening) |
| Framework Feedback (V00.74.10) | `InitialAdminCreationResult.InternalError` carries `Throwable cause` (was message-only, root cause was lost) | Shipped / V00.74.10 |
| Framework Feedback (V00.74.10) | `InitialAdminBootstrapService` propagates the original exception via SLF4J `logger().warn("...{}...", arg, e)` placeholder discipline | Shipped / V00.74.10 |
| Framework Feedback (V00.74.10) | `EmailVerificationService` + `PasswordResetService` — six `catch (RuntimeException ignored)` swallow-blocks rewritten to log via `HasLogger` (audit-sink failures no longer silently lost) | Shipped / V00.74.10 |
| Framework Feedback (V00.74.10) | `PasswordPolicy.minLength()` returns `OptionalInt` for `MinimumLengthPasswordPolicy` — UI feedback can show the limit instead of just rejecting | Shipped / V00.74.10 |
...
| AutoService | Maven Enforcer ban on mocking frameworks (`org.mockito:*`, `org.easymock:*`, `org.powermock:*`, `net.bytebuddy:byte-buddy-agent`) reactor-wide — Konzept-V00.74.20 §9 No-Mocks-Disziplin | Shipped / V00.74.20 |
...
| Credential Pipeline | Argon2id / bcrypt / scrypt providers in opt-in module `jSentinel-crypto-bc` (BouncyCastle 1.84 since V00.74.20; was 1.78.1 in V00.71–V00.74.10; no global JCA provider registration) | Shipped / V00.71; bcprov bumped V00.74.20 (closes Dependabot #14 + #15) |
...
| Credential Pipeline | `PasswordPolicy.minLength()` exposes `OptionalInt` for UI feedback (V00.74.10 framework-feedback fix) | Shipped / V00.74.10 |
...
| Account Lifecycle | `InitialAdminCreationResult.InternalError(cause)` propagation through `InitialAdminBootstrapService` (V00.74.10 framework-feedback fix) | Shipped / V00.74.10 |
| Account Lifecycle | `EmailVerificationService` + `PasswordResetService` audit-sink failure logging via `HasLogger` (V00.74.10 — six swallow-blocks closed) | Shipped / V00.74.10 |
...
| Compliance | Serialization policy — explicit no-Java-serialization stance (CWE-502 sidestep); 4 legitimate `Serializable` shapes documented (Vaadin lifecycle + `Throwable` heritage); Eclipse-Store / Canonical JSON / JOSE codec choice; V00.74.20 addendum closes the `users.ser`-fallback gap | Shipped / V00.74.10; V00.74.20 addendum (docs/security/credentials/standards/serialization-policy.md) |
...
| Storage-Pair (V00.74.20) | `StorageLayout(String frameworkSubdir, String appSubdir)` record — compact-constructor rejects null/blank/path-separator/NUL bytes + identical-subdir collisions (Konzept §6 code `persistence/storage-pair-subdir-collision`); `DEFAULT = ("jsentinel-store", "app-store")` | Shipped / V00.74.20 |
| Storage-Pair (V00.74.20) | `JSentinelStoragePair(framework, app, parent, layout, closedFlag)` — 5-component record `implements AutoCloseable, HasLogger`; two-phase `close()` with `addSuppressed`, `AtomicBoolean`-guarded idempotency | Shipped / V00.74.20 |
| Storage-Pair (V00.74.20) | `JSentinelStorageFactory.openAt(Path)` / `openAt(Path, StorageLayout)` — parent validation (Konzept §6 codes `persistence/storage-pair-parent-not-directory`, `…-parent-not-writable`); half-open rollback via `safelyShutdown` swallowing secondary failures | Shipped / V00.74.20 |
| Storage-Pair (V00.74.20) | `EclipseStoreJSentinelStorage.initStorageManager(Path)` shared init pipeline + new package-private 1-arg constructor (Public API `openAt(Path)` byte-compatible) | Shipped / V00.74.20 |
| Storage-Pair (V00.74.20) | Konzept §6 audit/log codes: `persistence/storage-pair-app-shutdown-failed` (WARN+re-throw), `…-framework-close-failed` (WARN+addSuppressed), `…-double-close` (INFO no-op) | Shipped / V00.74.20 |
| Storage-Pair (V00.74.20) | All Storage-Pair types `@ExperimentalJSentinelApi` until V00.75 confirms shape | Shipped / V00.74.20 |
...
| Demos (V00.74) | 10 `demo-jsentinel-*` skill demos (rest, rest-hardening, rest-persistence, standalone, standalone-hardening, standalone-persistence, vaadin, vaadin-hardening, vaadin-persistence, vaadin-mfa); parent-pom coordinated bump 00.73.00 → 00.74.20 at Phase 0 of V00.74.20 | Shipped / V00.74; aligned V00.74.20 |
| Demos (V00.74.20) | 3 persistence demos (vaadin / rest / standalone) migrated to `JSentinelStorageFactory.openAt(...)` — `JSentinelStorageProvider` holds `JSentinelStoragePair`, `UserDirectoryProvider` uses `pair.app()`, `EclipseStoreUserDirectoryPersistence` takes `EmbeddedStorageManager` instead of `Path`, one shutdown hook instead of two | Shipped / V00.74.20 |
| Demos (V00.74.20) | `demo-jsentinel-vaadin-hardening/views/admin/HealthView` — `@Route("admin/health")` `@RequiresPermission("admin:roles")`; renders `runtime.summary()` banner, health badges, `Grid<HealthFinding>`, prettified `runtime.toJson()`; refresh button reads `JSentinelBootstrapInitListener.currentRuntime()` | Shipped / V00.74.20 |
| Skills (V00.74.20) | 3 persistence skills (jsentinel-vaadin-persistence / jsentinel-rest-persistence / jsentinel-standalone-persistence) — `JSentinelStorageProvider.tmpl` + `EclipseStoreUserDirectoryPersistence.tmpl` + `UserDirectoryProvider.tmpl` + `AppStoragePaths.tmpl` migrated to the pair factory; `BootstrapWiring.tmpl` unchanged (AppStoragePaths is the indirection layer); both `~/.claude/skills/` and `docs/skills/claude/` mirrors synced | Shipped / V00.74.20 |
...
| Testing | PIT V00.74.10 re-measure (dx-standalone 63 %, dx-rest 70 %, dx-vaadin 53 %, vaadin-starter 38 %, autoservice-processor 52 %); two small gaps folded into V00.74.20, two large gaps stay backlog past V00.74.20 | Shipped / V00.74.10 |
| Testing (V00.74.20) | PIT measured at V00.74.20 release: `jSentinel-persistence-eclipsestore` **71 %** (247/350 kills, V00.74.10 was 70 %, held despite added Storage-Pair surface); `jSentinel-dx-standalone` **66 %** (39/59, target ≥ 65 % ✅); `jSentinel-autoservice-processor` **54 %** (35/65, target ≥ 65 % ⚠ missed) | Shipped / V00.74.20 |
| Testing (V00.74.20) | `ShutdownFailingStorageManager` test-support — JDK `Proxy` wrapper that throws on `shutdown()`; consistent with the V00.74.20 no-mocks Enforcer rule (it is a real-class proxy, not a mocking-framework construct) | Shipped / V00.74.20 |
| Testing (V00.74.20) | 6 `JSentinelStoragePairLifecycleTest` cases — happy-path / Phase-1-fail / Phase-2-fail / both-fail / double-close / try-with-resources | Shipped / V00.74.20 |
...
| Documentation | `docs/dx/decision-table.md` (`SecuredProxy` vs `@Secured` vs `SecuredUi` vs `@JSentinelAutoService` vs adapter bootstrap facades; +V00.74.20 row for Storage-Pair) | Shipped / V00.72; extended V00.74.20 |
| Documentation | jSentinel-themed Javadoc (`maven-javadoc-plugin` with `jsentinel.css` + Light/Dark toggle) | Shipped / V00.73; CSS path moved to `docs/javadoc/` in V00.74.20 (tracked) — V00.74.10 shipped EMPTY javadoc jars because the path was `.gitignore`d; bundle-script guard now fails build if any `-javadoc.jar` < 50 KB |
| Documentation | `Konzept-V00.74.10.md` + `Implementierungsplan-V00.74.10.md` + `RELEASE-NOTES-00.74.10.md` + GitHub Release page v00.74.10 | Shipped / V00.74.10 |
| Documentation | `Konzept-V00.74.20.md` + `Implementierungsplan-V00.74.20.md` + `RELEASE-NOTES-00.74.20.md` + GitHub Release page v00.74.20 (with `--latest` promotion) | Shipped / V00.74.20 |
| Documentation | `docs/estimates/google-login-effort-estimate.md` (3-path analysis: Spike 3–5 d / Lite 12–18 d / Roadmap 9–14 mo) | Shipped / 2026-06-15 |
| Documentation | `docs/estimates/google-calendar-integration-effort-and-concept.md` (incremental cost on top of login; V00.74 propagation already there) | Shipped / 2026-06-23 |
| Documentation | `docs/dx/google-calendar-framework-vs-app-boundary.md` (framework vs application layer split for Google-Calendar integration) | Shipped / 2026-06-23 |
| Documentation | 5-minute setup docs — §7 "Persistence pair" added to vaadin / rest / standalone | Shipped / V00.74.20 |
| Security Hygiene | bcprov-jdk18on 1.78.1 → 1.84 — closes Dependabot #14 (MED GHSA-c3fc-8qff-9hwx / CVE-2026-0636 LDAPStoreHelper injection) + #15 (HIGH GHSA-p93r-85wp-75v3 / CVE-2026-5598 FrodoEngine timing channel); neither code path is exercised in `jSentinel-crypto-bc` (we only use the low-level Argon2id / bcrypt / scrypt primitives), so real exploitation surface is zero — hygiene bump | Shipped / V00.74.20 |
| Security Hygiene | Maven-Central `-javadoc.jar` regression fix — V00.74.10 shipped empty javadoc jars to Central (immutable, stays that way); V00.74.20+ produces filled javadoc jars again via tracked CSS path + 50 KB bundle-script guard | Shipped / V00.74.20 |
| Release (V00.74.20) | Maven Central published 2026-06-23 — `com.svenruppert.jsentinel:*:00.74.20` (21 deployable modules, 162 primary files, 8.3 MB bundle, all GPG-signed); deployment ID `594cdd77-bb48-439b-bb26-ddcda8cd4c65`; Sonatype USER_MANAGED promotion from VALIDATED to PUBLISHED via UI; 0 errors, 2 INFO warnings on Sonatype org-quota limits (enforcement begins 2026-08-11) | Shipped / V00.74.20 |
| Release (V00.74.20) | GitHub Release page live at https://github.com/vaadin-developer/security-for-flow/releases/tag/v00.74.20 with `--latest` promotion | Shipped / V00.74.20 |
| V00.75 (planned) | Security event bus + dedicated `jSentinel-events` module | Concept (Konzept-V00.75.00.md) |
| V00.75 (planned) | REST/SSE bridge, signed event envelopes (Ed25519), replay protection, sequencing per `tenantId + producerId` | Concept |
| V00.75 (planned) | Producer policy, dead-letter store, event-replay store, event-sequence store | Concept |
| V00.75 (planned) | Canonical JSON + Eclipse-Serializer payload codecs; audit migrated to event-bus listener | Concept |
| V00.75 (planned) | Full 5-Minute-Setup pages for V00.74 `.propagation(...)` | Deferred from V00.74 |
| V00.75 (planned) | `demo-vaadin-rest-client` view-code migration to `@PropagateToken`-annotated client interfaces | Deferred from V00.74 |
| V00.75 (planned) | Storage-Pair sub-builder `.storage(p -> p.openAt(dir))` on `*Security.bootstrap()` candidate (Konzept §7 V00.74.20 explicit non-scope, earliest V00.75) | Concept |
| V00.76 (concept) | `jSentinel-jwt` — JOSE / JWT crypto base: `JwtValidator`, `AlgorithmAllowList` (RS256 / ES256 default), `JwksClient` with cache + 5-step rotation, `ClaimsValidator` + `ClockSkewPolicy` | Concept (Konzept-V00.76.00.md, vollständig) |
| V00.76 (concept) | `JwtToken` sealed subtype of V00.74 `AuthenticationToken`; Bootstrap sub-builder `.jwt(...)` | Concept |
| V00.76 (concept) | Stable-API promotion of V00.74 token-propagation types (after OIDC-Lackmus in V00.78) | Concept |
| V00.76 (concept) | `RefreshableTokenCredentialStore extends TokenCredentialStore` + automatic refresh-token rotation | Concept |
| V00.76 (concept) | Reactive `CompletionStage<HeaderValue>` strategy form | Concept |
| V00.77 (concept) | `jSentinel-oauth2` — OAuth2 RP-Flows: Authorization Code Flow + PKCE, Refresh, Revocation, Introspection, Client-Auth-Methods (`client_secret_basic` / `client_secret_post` / `private_key_jwt` / `client_secret_jwt` / `none`), `StateStore`, sealed `TokenResponse` / `OAuth2Error` | Concept (Konzept-V00.77.00.md, vollständig) |
| V00.77 (concept) | Bootstrap sub-builder `.oauth2(...)`; `StateStore` for state + PKCE-Verifier + optional nonce | Concept |
| V00.78 (concept) | `jSentinel-identity-oidc` — OIDC RP: Discovery (`.well-known/openid-configuration` + RFC 8414), ID-Token validation (`nonce`/`at_hash`/`azp`/`acr`), `UserInfoClient`, RP-initiated Logout, `ClaimsToSubjectMapper`, six new audit events | Concept (Konzept-V00.78.00.md, vollständig) |
| V00.78 (concept) | Discovery-driven bootstrap (`.oidc(o -> o.issuer(...))` resolves all OAuth2 endpoints + JWKS-URI) | Concept |
| V00.79 (concept) | Vendor profiles: Keycloak, Entra ID (Azure AD), Auth0, Okta, **Google**, GitHub (`jSentinel-identity-vendor-<name>` opt-in modules) | Concept (Konzept-V00.79.00.md, vollständig) |
| V00.79 (concept) | Logout-Hardening: Back-Channel / Front-Channel / OIDC Session Management 1.0 | Concept |
| V00.79 (concept) | DPoP (RFC 9449) with `cnf`-claim verification + replay protection | Concept |
| V00.80 | MFA / step-up authentication | Planned |
| V00.80 | TOTP / recovery codes | Planned |
| V00.80 | WebAuthn / passkeys | Planned |
| V00.80 | OIDC / OAuth2 bridge | Superseded by V00.76–V00.79 federation roadmap |
...
| V00.80 | Monitoring / metrics / health | Partially in V00.71 (`CredentialSecurityMetrics` SPI); `JSentinelRuntime.healthCheck()` shipped in V00.74.10; `HealthView` demo view shipped in V00.74.20; export adapters Planned |
| V00.80 | Fail-closed strict mode | Delivered in V00.72 (`JSentinelBootstrapMode.STRICT` + `JSentinelBootstrapException`) |
| V00.80 | Supply-chain / release hardening | Partially in V00.71 (CycloneDX SBOM per module, FIPS profile docs, SBOM verification expectations); serialization-policy.md added V00.74.10 (+ V00.74.20 addendum); `-javadoc.jar` guard added V00.74.20 |
| V00.80 | CSRF / web-adapter hardening | Planned |
| V00.80 | Privacy / retention | Planned |
| V00.80 | Backup / restore | Planned |
| Backlog (past V00.74.20) | `jSentinel-autoservice-processor` mutation-lift to ≥ 65 % (V00.74.20 hit 54 %, file-merge test infrastructure carried over — needs two-round InMemoryCompiler API or temp-dir integration-style processor tests) | Documented backlog (RELEASE-NOTES-00.74.20.md) |
| Backlog (past V00.74.20) | `jSentinel-dx-vaadin` mutation-lift 53 % → 75 % and `jSentinel-vaadin-starter` 38 % → 75 % — structural gaps, not incremental; explicit Konzept-V00.74.20 §7a non-scope | Documented backlog |
