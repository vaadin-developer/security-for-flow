...
/**
 * Aggregates {@link JCustosHealthIndicator}s into one
 * {@link HealthStatus} — the runtime counterpart of the bootstrap-time
 * {@code JCustosRuntime.healthCheck()}.
 *
 * <p>Indicators are invoked in sorted {@link JCustosHealthIndicator#id()}
 ...
 * {@code JCustosRuntime.healthCheck()} severity rules: any
...
public final class JCustosHealthCheck {
   ...
   * {@link JCustosHealthIndicator} that failed to load during
   ...
   */
  public static HealthStatus check(List<JCustosHealthIndicator> indicators,
  ...
  public static HealthStatus discoverAndCheck() {
    List<JCustosHealthIndicator> indicators = new ArrayList<>();
    List<HealthFinding> loadFailures = new ArrayList<>();
    drainResilient(ServiceLoader.load(JCustosHealthIndicator.class).iterator(),
        indicators, loadFailures);
    return aggregate(indicators, loadFailures, Instant::now);
  }

  private static HealthStatus aggregate(List<JCustosHealthIndicator> indicators,
      List<HealthFinding> preFindings, Supplier<Instant> clock) {
    List<JCustosHealthIndicator> sorted = new ArrayList<>(indicators);
    sorted.sort(Comparator.comparing(JCustosHealthCheck::safeIndicatorId));
    List<HealthFinding> findings = new ArrayList<>(preFindings);
    for (JCustosHealthIndicator indicator : sorted) {
    ...
    // exactly the JCustosRuntime.healthCheck() severity rules:
  ...
  private static List<HealthFinding> checkOne(JCustosHealthIndicator indicator) {
    try {
  ...
  private static HealthFinding indicatorFailure(JCustosHealthIndicator indicator,
      String summary) {
   ...
   */
  private static String safeIndicatorId(JCustosHealthIndicator indicator) {
    try {
   ...
   * of spinning.
   */
  private static void drainResilient(Iterator<JCustosHealthIndicator> source,
      List<JCustosHealthIndicator> indicators, List<HealthFinding> loadFailures) {
