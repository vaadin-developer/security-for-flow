 ...
 * @since 00.72.00
 */
public final class JCustosDiagnostics {
  ...
  private JCustosDiagnostics() {
  }

  /**
   * @return a fresh diagnostic report
   */
  public static JCustosServiceReport inspect() {
    ClassLoader cl = Thread.currentThread().getContextClassLoader();
    if (cl == null) {
      cl = JCustosDiagnostics.class.getClassLoader();
    }
    return inspect(cl);
  }
   ...
   */
  static JCustosServiceReport inspect(ClassLoader cl) {
    ...
    for (Class<?> spi : CORE_SPIS) {
      ...
      if (impls.isEmpty() && CRITICAL.contains(spi)) {
        builder.addMissing(new MissingRecommendedService(
            ...
            "Register an implementation via @JCustosAutoService(" + spi.getSimpleName()
    ...
    // 2. derived rule: JCustosVersionStore present but SubjectIdResolver missing
    boolean hasVersionStore = builder.discovered.stream()
        .anyMatch(d -> d.spi() == JCustosVersionStore.class);
    ...
    if (hasVersionStore && !hasSubjectIdResolver) {
      builder.addWarning(new ServiceWarning(
          ...
          "JCustosVersionStore is registered, but no SubjectIdResolver was found.",
          "Register a SubjectIdResolver via @JCustosAutoService("
    ...
    contributors.sort(Comparator.comparing(JCustosDiagnostics::safeContributorId));
  ...
  private static final class InternalBuilder implements DiagnosticReportBuilder {
    ...
    JCustosServiceReport build() {
      return new JCustosServiceReport(
          discovered, missing, duplicates, warnings,
          processorReport == null ? JCustosProcessorReport.empty() : processorReport);
    }
  }
}
