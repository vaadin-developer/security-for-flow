...
import eu.jsentinel.jcustos.demo.skill.vaadin.security.model.UserDirectory;
...
import eu.jsentinel.jcustos.demo.skill.vaadin.security.services.PasswordPreflight;
import eu.jsentinel.jcustos.demo.skill.vaadin.security.services.VersionBumper;
...
/**
 * Hardening-skill replacement: same UI, but every mutation now calls
 * {@link VersionBumper#bump(Object)} so the affected user's open
 * sessions reroute to login on the next navigation, and new passwords
 * pass through {@link PasswordPreflight}.
 */
...
public class AdminRolesView extends Composite<VerticalLayout> {
  ...
  private HorizontalLayout buildRoleEditor(User user) {
    ...
    Button assign = new Button("Assign", VaadinIcon.PLUS.create(), e -> {
      ...
      success("Granted " + role.name() + " to " + user.name()
          + " — their session will reroute to login on the next click.");
      refresh();
    });
  ...
  private void openCreateUserDialog() {
    ...
    Button save = new Button("Create", VaadinIcon.CHECK.create(), e -> {
      ...
      // V00.71 password-preflight: local blocklist + (optional) HIBP.
      // Generic rejection message — never name the dictionary (CWE-209).
      if (!PasswordPreflight.isAcceptable(p)) {
        warn("Password rejected: this password is on a known-bad list. Choose another.");
        return;
      }
