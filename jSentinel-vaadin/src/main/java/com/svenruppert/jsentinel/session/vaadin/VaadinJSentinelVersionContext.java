 ...
 * {@link JSentinelVersionEnforcerListener Vaadin enforcer
 * listener} consults this context on every navigation; the
 * application records it once after a successful login via
 * {@link #record(SubjectId, TenantId, JSentinelVersion, String) record}.
 *
 ...
 * {@code JSentinelVersionStore} itself. Apps that want
 * "capture-current-on-login" semantics pair this helper with a
 * call to
 * {@code versionStore.current(new JSentinelVersionKey(tenant, subjectId))}
 * right before {@link #record record}.
 */
@ExperimentalJSentinelApi
public final class VaadinJSentinelVersionContext {

  private VaadinJSentinelVersionContext() {
    // utility
  }
