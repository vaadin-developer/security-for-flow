...
/**
 * Result of running
 * {@link OpenApiJCustosMetadataGenerator#generate} on a handler
 * class. Carries the class-level and per-method
 * {@link JCustosRequirement}s the generator extracted.
 * <p>
...
public record HandlerJCustosMetadata(
    String handlerClassName,
    List<JCustosRequirement> classLevel,
    Map<String, List<JCustosRequirement>> methods
) {

  /** Validates the components and freezes the collections. */
  public HandlerJCustosMetadata {
    ...
    var copy = new java.util.LinkedHashMap<String, List<JCustosRequirement>>(methods.size());
    for (var entry : methods.entrySet()) {
      String name = entry.getKey();
      if (name == null || name.isBlank()) {
        throw new IllegalArgumentException("methods keys must not be blank");
      }
      List<JCustosRequirement> reqs = entry.getValue();
