 ...
 * {@link SecurityVersionEnforcerListener Vaadin enforcer
 * listener} consults this context on every navigation; the
 * application records it once after a successful login via
 * {@link #record(SubjectId, TenantId, SecurityVersion, String) record}.
 *
 ...
 * {@code SecurityVersionStore} itself. Apps that want
 * "capture-current-on-login" semantics pair this helper with a
 * call to
 * {@code versionStore.current(new SecurityVersionKey(tenant, subjectId))}
 * right before {@link #record record}.
 */
@ExperimentalSecurityApi
public final class VaadinSecurityVersionContext {

  private VaadinSecurityVersionContext() {
    // utility
  }
