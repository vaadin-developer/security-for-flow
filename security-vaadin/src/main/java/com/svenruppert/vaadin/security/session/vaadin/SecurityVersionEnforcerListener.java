...
/**
 * Vaadin adapter for {@link SecurityVersionEnforcer}.
 * <p>
 ...
 * snapshot from {@link VaadinSecurityVersionContext}; sessions
 * without a recorded snapshot pass through untouched (the
 * application opted out by not calling
 * {@link VaadinSecurityVersionContext#record record} after login).
 *
 * <p>On drift:
 * <ol>
 *   <li>{@link SecurityVersionEnforcer#enforce enforce} publishes
...
public final class SecurityVersionEnforcerListener implements BeforeEnterListener {

  private final SecurityVersionEnforcer enforcer;
   ...
   */
  public SecurityVersionEnforcerListener(SecurityVersionEnforcer enforcer,
  ...
  public void beforeEnter(BeforeEnterEvent event) {
    Optional<VaadinSecurityVersionContext.Snapshot> snapshotOpt =
        VaadinSecurityVersionContext.current();
    if (snapshotOpt.isEmpty()) {
      return;
    }
    VaadinSecurityVersionContext.Snapshot snapshot = snapshotOpt.get();
    ...
    if (outcome instanceof EnforcementOutcome.SessionStale) {
      ...
      VaadinSecurityVersionContext.clear(VaadinSession.getCurrent());
