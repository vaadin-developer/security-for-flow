...
| 4 — Store-backed Services + SecurityVersion | teilweise (4a ✓: `SecurityVersionStore` + `SecurityVersionKey` in `session/`, In-Memory-Default mit atomic-compute, `SecurityVersionStoreContract` im testkit (9 Cases), `EclipseStoreSecurityVersionStore` + Root-Erweiterung, `EclipseStoreSecurityStorage.securityVersionStore()`-Accessor — beide Default-Impls passen 9 Contract-Cases bit-for-bit. **4b ✓**: alle sechs Store-backed Services in security-core implementiert und getestet — `StoreBackedSecurityAuditService` (7 Tests), `StoreBackedLoginAttemptPolicy` (7), `StoreBackedSubjectSessionRegistry` (7), `StoreBackedRoleAuthorizationService` (6), `StoreBackedRememberMeService` (8), `StoreBackedBootstrapStateService` (7); insgesamt 42 neue Tests, 851 Tests im Modul gruen. **4c offen**: `SecurityVersionCheck`-Interceptor in security-vaadin/-rest, `LogoutScope.AllSessionsOfSubject` store-backed, Role-Refresh-Demo-Exit-Test.) |
