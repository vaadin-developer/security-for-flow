...
| 3 — Contract Testkit + Eclipse Store | teilweise (3a-1 ✓: neues Modul `security-persistence-testkit`, drei Contract-Interfaces (`AuditEventStoreContract`, `SessionStoreContract`, `LoginAttemptStoreContract`) als Vorlage mit JUnit-5-`@Test default`-Methoden, InMemory-Adapter-Tests verifizieren das Pattern. 8 weitere Contracts (RoleAssignment, BootstrapState, RememberMe/PasswordReset/EmailVerification, ApiKey/RefreshToken/RateLimit) + Sub-Phase 3b (Eclipse-Store-Modul `security-persistence-eclipsestore` auf `org.eclipse.store:storage-embedded:4.1.0`) noch offen.) |
