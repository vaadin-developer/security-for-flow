...
/**
 * A device-authorization grant was denied or expired (RFC 8628 {@code access_denied}
 * / {@code expired_token}, V00.77). Carries only the stable reason code.
 *
 * @param metadata variable per-instance metadata
 * @param reason   the stable reason ({@code access_denied} / {@code expired_token})
 * @since 00.77.00
 */
@ExperimentalJSentinelApi
public record DeviceAuthorizationDeniedEvent(EventMetadata metadata, String reason)
    implements JSentinelEvent {

  public static final EventType TYPE = EventType.of("DeviceAuthorizationDenied");

  public DeviceAuthorizationDeniedEvent {
