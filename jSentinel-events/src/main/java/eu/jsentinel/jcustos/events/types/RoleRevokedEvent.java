...
/**
 * A role was revoked from a subject (Konzept §256). Often paired with a
 * security-version bump that forces affected sessions to reload.
 *
 * @param metadata variable per-instance metadata
 * @param role the revoked role name
 * @since 00.75.00
 */
@ExperimentalJSentinelApi
public record RoleRevokedEvent(EventMetadata metadata, String role)
    implements JSentinelEvent {

  public static final EventType TYPE = EventType.of("RoleRevoked");
  ...
  public JSentinelEventCategory category() {
    return JSentinelEventCategory.ROLE;
  }
}
