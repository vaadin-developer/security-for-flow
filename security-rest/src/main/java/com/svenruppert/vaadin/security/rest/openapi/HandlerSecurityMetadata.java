...
/**
 * Result of running
 * {@link OpenApiSecurityMetadataGenerator#generate} on a handler
 * class. Carries the class-level and per-method
 * {@link SecurityRequirement}s the generator extracted.
 * <p>
...
public record HandlerSecurityMetadata(
    String handlerClassName,
    List<SecurityRequirement> classLevel,
    Map<String, List<SecurityRequirement>> methods
) {

  /** Validates the components and freezes the collections. */
  public HandlerSecurityMetadata {
    ...
    var copy = new java.util.LinkedHashMap<String, List<SecurityRequirement>>(methods.size());
    for (var entry : methods.entrySet()) {
      String name = entry.getKey();
      if (name == null || name.isBlank()) {
        throw new IllegalArgumentException("methods keys must not be blank");
      }
      List<SecurityRequirement> reqs = entry.getValue();
