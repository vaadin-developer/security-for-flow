...
/**
 * REST adapter for {@link SecurityVersionEnforcer}.
 * <p>
 ...
 * {@link RestSecurityVersionContext} (the default), the filter
 * returns {@code true} unconditionally — applications that don't
 * yet track per-session versions opt in by overriding
 * {@link RestSubjectResolver#resolveSecurityVersionContext(RestRequest) resolveSecurityVersionContext}.
 */
@ExperimentalSecurityApi
public final class RestSecurityVersionFilter {
   ...
   */
  public RestSecurityVersionFilter(RestSubjectResolver resolver,
                                   SecurityVersionEnforcer enforcer) {
   ...
   *                 {@link SecurityVersionEnforcer#enforce enforce}
  ...
  public boolean allow(RestRequest request, RestResponse response, String route) {
    ...
    Optional<RestSecurityVersionContext> ctx =
        resolver.resolveSecurityVersionContext(request);
    if (ctx.isEmpty()) {
      return true;
    }
    RestSecurityVersionContext context = ctx.get();
