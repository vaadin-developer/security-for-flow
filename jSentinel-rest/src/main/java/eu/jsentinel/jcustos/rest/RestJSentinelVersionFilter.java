...
import eu.jsentinel.jcustos.session.JSentinelVersionEnforcer.EnforcementOutcome;
...
/**
 * REST adapter for {@link JSentinelVersionEnforcer}.
 * <p>
 ...
 * {@link RestJSentinelVersionContext} (the default), the filter
 * returns {@code true} unconditionally — applications that don't
 * yet track per-session versions opt in by overriding
 * {@link RestSubjectResolver#resolveJSentinelVersionContext(RestRequest) resolveJSentinelVersionContext}.
 */
@ExperimentalJSentinelApi
public final class RestJSentinelVersionFilter {
   ...
   */
  public RestJSentinelVersionFilter(RestSubjectResolver resolver,
                                   JSentinelVersionEnforcer enforcer) {
   ...
   *                 {@link JSentinelVersionEnforcer#enforce enforce}
  ...
  public boolean allow(RestRequest request, RestResponse response, String route) {
    ...
    Optional<RestJSentinelVersionContext> ctx =
        resolver.resolveJSentinelVersionContext(request);
    if (ctx.isEmpty()) {
      return true;
    }
    RestJSentinelVersionContext context = ctx.get();
