 ...
 * {@link JSentinelVersionCheck}. Adapters call
 * {@link #enforce(SubjectId, TenantId, JSentinelVersion, String, String)
...
public final class JSentinelVersionEnforcer {

  private final JSentinelVersionCheck check;
   ...
   *                     non-null
   */
  public JSentinelVersionEnforcer(JSentinelVersionStore store,
                                 JSentinelAuditService auditService) {
    this(new JSentinelVersionCheck(store), auditService, Clock.systemUTC());
  }
   ...
   */
  public JSentinelVersionEnforcer(JSentinelVersionCheck check,
                                 JSentinelAuditService auditService,
                                 Clock clock) {
  ...
  public EnforcementOutcome enforce(SubjectId subjectId,
    ...
    JSentinelVersionStatus status =
        check.check(new JSentinelVersionKey(effective, subjectId), snapshot);
    return switch (status) {
      case JSentinelVersionStatus.Current ignored -> EnforcementOutcome.CONTINUE;
      case JSentinelVersionStatus.Drifted drifted -> {
  ...
  public sealed interface EnforcementOutcome {
     ...
     */
    record SessionStale(JSentinelVersionStatus.Drifted status) implements EnforcementOutcome {
