...
class SecurityServiceResolverPolicyTest {
  ...
  void fallbackIsCached() {
    PolicyRegistry first = SecurityServiceResolver.policyRegistry();
    PolicyRegistry second = SecurityServiceResolver.policyRegistry();
  ...
  void findReturnsEmptyForFallback() {
    SecurityServiceResolver.policyRegistry(); // forces fallback caching
    assertTrue(SecurityServiceResolver.findPolicyRegistry().isEmpty());
  }

  @Test
  @DisplayName("setPolicyRegistry overrides the cached fallback")
  void setOverridesFallback() {
    SecurityServiceResolver.policyRegistry(); // cache fallback
    PolicyRegistry custom = new InMemoryPolicyRegistry();
    SecurityServiceResolver.setPolicyRegistry(custom);
    assertSame(custom, SecurityServiceResolver.policyRegistry());
  }
  ...
  void setNullClears() {
    PolicyRegistry custom = new InMemoryPolicyRegistry();
    SecurityServiceResolver.setPolicyRegistry(custom);
    SecurityServiceResolver.setPolicyRegistry(null);

    PolicyRegistry next = SecurityServiceResolver.policyRegistry();
  ...
  void resetAllClearsRegistry() {
    ...
    assertInstanceOf(InMemoryPolicyRegistry.class, next);
    assertNotNull(next);
    assertSame(next, SecurityServiceResolver.policyRegistry());
  }
}
