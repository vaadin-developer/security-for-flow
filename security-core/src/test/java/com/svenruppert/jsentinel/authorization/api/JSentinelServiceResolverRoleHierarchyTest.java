...
import com.svenruppert.jsentinel.authorization.api.roles.StaticRoleHierarchy;
...
class JSentinelServiceResolverRoleHierarchyTest {
  ...
  void fallbackIsNoopInstance() {
    RoleHierarchy hierarchy = JSentinelServiceResolver.roleHierarchy();
  ...
  void findReturnsEmptyForFallback() {
    JSentinelServiceResolver.roleHierarchy(); // forces fallback caching
    assertTrue(JSentinelServiceResolver.findRoleHierarchy().isEmpty());
  }

  @Test
  @DisplayName("setRoleHierarchy overrides the cached fallback")
  void setOverridesFallback() {
    JSentinelServiceResolver.roleHierarchy(); // cache fallback
    RoleHierarchy custom = StaticRoleHierarchy.builder()
        .role(new RoleName("ROLE_ADMIN")).inheritsFrom(new RoleName("ROLE_EDITOR"))
        .build();
    JSentinelServiceResolver.setRoleHierarchy(custom);
    assertSame(custom, JSentinelServiceResolver.roleHierarchy());
    assertTrue(JSentinelServiceResolver.findRoleHierarchy().isPresent());
  }

  @Test
  @DisplayName("setRoleHierarchy(null) clears the cache, Noop returns again")
  void setNullClears() {
    JSentinelServiceResolver.setRoleHierarchy(
        StaticRoleHierarchy.builder()
            .role(new RoleName("ROLE_ADMIN")).inheritsFrom(new RoleName("ROLE_EDITOR"))
            .build());
    JSentinelServiceResolver.setRoleHierarchy(null);
    assertSame(NoopRoleHierarchy.INSTANCE, JSentinelServiceResolver.roleHierarchy());
  }
